...
export const XaiPanel: React.FC = () => {
  ...
  const drivers: XaiDriver[] = useMemo(() => {
    ...
    if (
      xai.top_diagnostic_drivers &&
      xai.top_diagnostic_drivers.length > 0
    ) {
      return xai.top_diagnostic_drivers.map((d) => ({
        ...
        impact:
          d.impact ||
          (d.attribution_score && d.attribution_score > 0.05
            ? 'ELEVATED'
            : 'NOMINAL'),
        attribution_score:
          d.attribution_score ??
          d.importance ??
          Math.abs(d.shap_value || 0),
        direction: d.direction || 'increases_likelihood',
        direction_text:
          d.direction_text || `${d.sensor} influence`,
      }));
    }

    // 2. Extract from fault model top_contributors
    if (
      xai.fault?.top_contributors &&
      xai.fault.top_contributors.length > 0
    ) {
      return xai.fault.top_contributors.map((c) => ({
        sensor:
          ...
          'Telemetry Signal',
        impact:
          Math.abs(c.shap_value || 0) > 0.2
            ? 'HIGH_IMPACT'
            : 'MODERATE',
        attribution_score: Math.abs(
          c.shap_value || c.importance || 0
        ),
    ...
    if (
      xai.anomaly?.top_contributors &&
      xai.anomaly.top_contributors.length > 0
    ) {
    ...
    return [
      {
        ...
        direction: 'neutral',
        direction_text:
          'Within calibrated nominal thermodynamic envelope',
      },
      {
        ...
        direction: 'neutral',
        direction_text:
          'Combustion exhaust baseline normal',
      },
      {
        ...
        direction: 'neutral',
        direction_text:
          'Governed cruising speed matched to throttle demand',
      },
    ];
  }, [xai]);
  ...
  const maxScore = Math.max(
    ...drivers.map((d) => d.attribution_score || 0.01),
    0.05
  );
    ...
    <div className="bg-avionics-surface border border-avionics-border rounded-lg p-3.5 space-y-3">
      ...
      {diagnostic && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          ...
          {/* Fault ID */}
          <div className="bg-avionics-card rounded border border-slate-800 p-2">
            <div className="text-[9px] font-mono text-slate-500 uppercase">
              Fault ID
            </div>

            <div className="text-sm font-mono font-bold text-cyan-300 mt-0.5">
              {diagnostic.fault_id || 'NORMAL'}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
        ...
        {/* Left: Ranked Diagnostic Drivers */}
          ...
          <div className="space-y-2 pt-1">
            {drivers.slice(0, 5).map((d, idx) => {
              ...
              const barWidth = Math.min(
                100,
                Math.max(5, (score / maxScore) * 100)
              );

              const isHigh =
                score > 0.2 ||
                (d.impact && d.impact.includes('HIGH'));

              const isWarning =
                score > 0.08 ||
                (d.impact && d.impact.includes('ELEVATED'));
        ...
        {/* Right: Engineering Interpretation & Actions */}
